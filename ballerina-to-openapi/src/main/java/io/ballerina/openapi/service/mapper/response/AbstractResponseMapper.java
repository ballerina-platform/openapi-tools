...
public abstract class AbstractResponseMapper implements ResponseMapper {
    ...
    protected final ApiResponses apiResponses = new ApiResponses();
    ...
    public void setApiResponses(OperationInventory operationInventory, FunctionDefinitionNode resource) {
        extractAnnotationDetails(resource);
        String defaultStatusCode = operationInventory.getHttpOperation().equalsIgnoreCase(POST) ? HTTP_201 : HTTP_200;
        String mediaTypeSubTypePrefix = MediaTypeUtils.extractCustomMediaType(resource).orElse("");
        for (TypeSymbol returnTypeSymbol: getReturnTypes(resource)) {
            createResponseMapping(returnTypeSymbol, defaultStatusCode, mediaTypeSubTypePrefix);
        }
    ...
    private void createResponseMapping(TypeSymbol returnType, String defaultStatusCode, String mediaTypeSubTypePrefix) {
        ...
        if (Objects.nonNull(unionType)) {
            addResponseMappingForUnion(defaultStatusCode, unionType, mediaTypeSubTypePrefix);
        } else {
            addResponseMappingForSimpleType(returnType, defaultStatusCode, mediaTypeSubTypePrefix);
        }
    }

    private UnionTypeSymbol getUnionType(TypeSymbol typeSymbol, SemanticModel semanticModel) {
    ...
    private void addResponseMappingForUnion(String defaultStatusCode, UnionTypeSymbol unionType,
        ...
        Map<String, Map<String, TypeSymbol>> responseCodeMap = getResponseCodeMap(
                unionType, defaultStatusCode, mediaTypeSubTypePrefix);
    ...
    private void addResponseMappingForSimpleType(TypeSymbol returnType, String mediaTypeSubTypePrefix,
    ...
    public Map<String, Map<String, TypeSymbol>> getResponseCodeMap(UnionTypeSymbol typeSymbol, String defaultCode,
        ...
        extractBasicMembers(typeSymbol, defaultCode, mediaTypeSubTypePrefix, responsesCodeMap);
    ...
    private void extractBasicMembers(UnionTypeSymbol unionTypeSymbol, String defaultCode, String mediaTypeSubTypePrefix,
        ...
        for (TypeSymbol directMemberType : directMemberTypes) {
            ...
            if (isSameMediaType(directMemberType, semanticModel)) {
                ...
                extractBasicMembers(unionType, code, mediaTypeSubTypePrefix, responses);
            }
        }
    }
    ...
    private void updateApiResponseContentWithMediaType(ApiResponse apiResponse, Content content) {
    ...
    public boolean isHttpStatusCodeResponseType(SemanticModel semanticModel, TypeSymbol typeSymbol) {
    ...
    private TypeSymbol getBodyTypeFromStatusCodeResponse(TypeSymbol typeSymbol, SemanticModel semanticModel) {
    ...
    private Map<String, Header> getHeadersFromStatusCodeResponse(TypeSymbol typeSymbol) {
        RecordTypeSymbol statusCodeRecordType = getStatusCodeRecordTypeSymbol(typeSymbol);
    ...
    private String getResponseCode(TypeSymbol typeSymbol, String defaultCode, SemanticModel semanticModel) {
        if (isSubTypeOfNil(typeSymbol, semanticModel)) {
            return HTTP_202;
    ...
    private boolean isSubTypeOfNil(TypeSymbol typeSymbol, SemanticModel semanticModel) {
    ...
    private boolean isSubTypeOfError(TypeSymbol typeSymbol, SemanticModel semanticModel) {
    ...
    private boolean isSubTypeOfHttpStatusCodeResponse(TypeSymbol typeSymbol, SemanticModel semanticModel) {
        return isSubTypeOfHttpRecordType("StatusCodeResponse", typeSymbol, semanticModel);
    }

    private boolean isSubTypeOfHttpRecordType(String type, TypeSymbol typeSymbol, SemanticModel semanticModel) {
        Optional<Symbol> optionalRecordSymbol = semanticModel.types().getTypeByName("ballerina", "http",
                "", type);
        if (optionalRecordSymbol.isPresent() &&
