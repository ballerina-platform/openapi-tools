...
/**
 * Ballerina OpenAPI example annotation(s) analyzer for record fields.
 *
 * @since 2.1.0
 */
public class RecordFieldExampleAnalyzer extends AbstractExampleAnalyzer
    ...
    public void perform(SyntaxNodeAnalysisContext context) {
        ...
        if (symbol.isEmpty() || !(symbol.get() instanceof RecordFieldSymbol typeDefinition)) {
            return;
        }

        List<AnnotationAttachmentSymbol> annotations = typeDefinition.annotAttachments();
        TypeSymbol typeSymbol = typeDefinition.typeDescriptor();
