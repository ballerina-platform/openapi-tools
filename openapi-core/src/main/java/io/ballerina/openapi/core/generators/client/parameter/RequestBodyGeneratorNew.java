...
public class RequestBodyGeneratorNew implements ParameterGenerator {
    OpenAPI openAPI;
    RequestBody requestBody;
    List<ClientDiagnostic> diagnostics = new ArrayList<>();
    List<Parameter> headerSchemas = new ArrayList<>();

    public RequestBodyGeneratorNew(RequestBody requestBody, OpenAPI openAPI) {
            ...
            if (headers != null) {
                for (Map.Entry<String, Header> header : headers.entrySet()) {
                    if (!headerList.contains(escapeIdentifier(header.getKey()))) {
                        RequestBodyHeaderParameterNew requestBodyHeaderParameter = new RequestBodyHeaderParameterNew(
                                header);
