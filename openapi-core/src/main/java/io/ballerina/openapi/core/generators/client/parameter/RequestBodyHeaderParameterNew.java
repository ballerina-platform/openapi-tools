...
public class RequestBodyHeaderParameterNew {
    Map.Entry<String, Header> header;
    List<ClientDiagnostic> diagnostics = new ArrayList<>();

    RequestBodyHeaderParameterNew(Map.Entry<String, Header> header) {
        this.header = header;
    }
    ...
    public Optional<Parameter> generateParameterNode(boolean treatDefaultableAsRequired) {
        Schema schema = getSchemaWithDescription(header.getValue(), treatDefaultableAsRequired);
    ...
    private Schema getSchemaWithDescription(Header header, boolean treatDefaultableAsRequired) {
