...
public class OpenAPICmdTest extends OpenAPICommandTest {
    ...
    public void testServiceGenerationWithoutDataBinding() throws IOException {
        Path sharedProjectDir = resourceDir.resolve("expected_gen").resolve("ballerina_project");
        Path projectDir = Files.createTempDirectory("openapi-service-gen-test-out-" + System.nanoTime());
        Files.copy(sharedProjectDir.resolve("Ballerina.toml"), projectDir.resolve("Ballerina.toml"));
        Files.copy(sharedProjectDir.resolve("Package.md"), projectDir.resolve("Package.md"));
        try {
            Path petstoreYaml = resourceDir.resolve(Paths.get("petstore.yaml"));
            String[] args = {"--input", petstoreYaml.toString(), "-o", projectDir.toString(),
