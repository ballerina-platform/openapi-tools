...
public client class Client {
    ...
    remote isolated function addonPropertiesResource.getAddonProperties_get(string addonKey) returns PropertyKeys|error {
        string  path = string `/rest/atlassian-connect/1/addons/${addonKey}/properties`;
        PropertyKeys response = check self.clientEp->get(path, targetType = PropertyKeys);
        return response;
    }
    remote isolated function addonPropertiesResource.getAddonProperty_get(string addonKey, string propertyKey) returns EntityProperty|error {
        string  path = string `/rest/atlassian-connect/1/addons/${addonKey}/properties/${propertyKey}`;
        EntityProperty response = check self.clientEp->get(path, targetType = EntityProperty);
        return response;
    }
    remote isolated function addonPropertiesResource.putAddonProperty_put(string addonKey, string propertyKey, json payload) returns OperationMessage|error {
        string  path = string `/rest/atlassian-connect/1/addons/${addonKey}/properties/${propertyKey}`;
        http:Request request = new;
        json jsonBody = check payload.cloneWithType(json);
        request.setPayload(jsonBody);
    ...
    remote isolated function addonPropertiesResource.deleteAddonProperty_delete(string addonKey, string propertyKey) returns http:Response | error {
        string  path = string `/rest/atlassian-connect/1/addons/${addonKey}/properties/${propertyKey}`;
        http:Request request = new;
        //TODO: Update the request as needed;
        http:Response  response = check self.clientEp->delete(path, request, targetType = http:Response );
    ...
    remote isolated function dynamicModulesResource.getModules_get() returns ConnectModules|error {
    ...
    remote isolated function dynamicModulesResource.registerModules_post(ConnectModules payload) returns http:Response | error {
        string  path = string `/rest/atlassian-connect/1/app/module/dynamic`;
        http:Request request = new;
        json jsonBody = check payload.cloneWithType(json);
        request.setPayload(jsonBody);
    ...
    remote isolated function dynamicModulesResource.removeModules_delete(string[]? moduleKey) returns http:Response | error {
        string  path = string `/rest/atlassian-connect/1/app/module/dynamic`;
