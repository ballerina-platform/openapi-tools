...
public isolated client class Client {
    ...
    remote isolated function getPetId(string petId) returns http:Response | error {
        string resourcePath = string `/pets/${getEncodedUri(petId.toString())}`;
        http:Response  response = check self.clientEp->get(resourcePath);
        return response;
    }
    remote isolated function  ImageByimageId(int petId, string imageId) returns http:Response | error {
        string resourcePath = string `/pets/${getEncodedUri(petId.toString())}/Image/${getEncodedUri(imageId.toString())}`;
        http:Response  response = check self.clientEp->get(resourcePath);
        return response;
    }
