...
service class RequestInterceptor {
    *http:RequestInterceptor;
	...
	resource function 'default [string... path](http:RequestContext ctx) returns string|http:NextService|error? {
		return ctx.next();
	}
}
...
service class ResponseErrorInterceptor {
    *http:ResponseErrorInterceptor;
	...
	remote function interceptResponseError(error err, http:RequestContext ctx) returns http:NextService? {
		return checkpanic ctx.next();
	}
}
